/* http://github.com/pmp-p  target pym:/data/cross/wapy/cmod/wasi/embed.pym */
/*
  embed.c AUTO-GENERATED by /data/git/wapy-lib/modgen/__main__.py
*/
...
// opt: no finally int slot
    ...
    // ------- method body (try/finally) -----
    { __creturn__ = (long)mp_obj_new_int_from_uint(VMFLAGS_IF); goto lreturn__; };
...
// opt: no finally int slot
    ...
    // ------- method body (try/finally) -----
    { __creturn__ = (long)mp_obj_new_int_from_uint(1); goto lreturn__; };
    { __creturn__ = (long)mp_obj_new_int_from_uint(0); goto lreturn__; };
lreturn__: return mp_obj_new_int(__creturn__);
}
...
// opt : void return
    ...
    // ('const_char_p', '""') => const char * = ""
        ...
        if (argc>0) { source_file = mp_obj_str_get_str(argv[0]); } else { source_file = mp_obj_new_str_via_qstr("",0); }
    
    
        const char *mpy_file; // ('const char *', 'mpy_file', '""')
        if (argc>1) { mpy_file = mp_obj_str_get_str(argv[1]); } else { mpy_file = mp_obj_new_str_via_qstr("",0); }
...
STATIC MP_DEFINE_CONST_FUN_OBJ_VAR_BETWEEN(embed_os_compile_obj, 0, 2, embed_os_compile);
...
// __main__
    ...
    {MP_OBJ_NEW_QSTR(MP_QSTR_os_compile), (mp_obj_t)&embed_os_compile_obj },
