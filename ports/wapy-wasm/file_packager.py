...
def make_hash(data):
    return __import__('hashlib').md5(data.encode('utf-8')).hexdigest()
...
try:
    ...
    raise SystemExit(f"{PYDIR} is not writeable")
...
try:
    ...
    #old and buggy
    try:
        from fstrings_helper import decode as fstring_decode
    except:
        raise SystemExit(f"future_fstrings[rewrite] or fstrings_helper not installed")
...
if len(sys.argv) == 1:
    print(
        """Usage: file_packager.py TARGET [--preload A [B..]] [--embed C [D..]] [--exclude E [F..]]] [--js-output=OUTPUT.js] [--no-force] [--use-preload-cache] [--indexedDB-name=EM_PRELOAD_CACHE] [--no-heap-copy] [--separate-metadata] [--lz4] [--use-preload-plugins]
...
def should_ignore(fullname, is_file=0):
  ...
  is hidden (Win32) or it matches any pattern specified in --exclude"""
    ...
    if is_file and fullname.endswith('.py'):
        process_filter = f"{PYDIR}/{make_hash(fullname)}"
        with open(fullname,'rb') as src, open(process_filter,'wb') as dst:
            content, _ = fstring_decode(src.read())
            dst.write( content.encode('UTF-8') )
        print(fullname, process_filter, file=sys.stderr)
    ...
    for dirpath, dirnames, filenames in os.walk(rootpathsrc):
        ...
        for name in dirnames:
            ...
            if not should_ignore(fullname):
                ...
                print(
                    'Skipping directory "%s" from inclusion in the emscripten ' "virtual file system." % fullname, file=sys.stderr
                )
        for name in filenames:
            fullname = os.path.join(dirpath, name)
            if not should_ignore(fullname, is_file=1):
                ...
                if process_filter is None:
                    ...
                    new_data_files.append({"srcpath": process_filter, "dstpath": dstpath, "mode": mode, "explicit_dst_path": True})
            elif DEBUG:
...
def main():
  ...
  """
        ...
        if not file_["explicit_dst_path"]:
            ...
            if os.path.isabs(path):
                print(
                    ...
                    "location the absolute source path should be directed to." % (path, file_["dstpath"]),
                    file=sys.stderr,
                )
    ...
    for file_ in data_files:
        ...
        if DEBUG:
            print('Packaging file "%s" to VFS in path "%s".' % (file_["srcpath"], file_["dstpath"]), file=sys.stderr)
